angular.module('mean.system').controller('LeadsController', ['$scope', 'Global', 'Leads', function ($scope, Global, Leads) {
    $scope.global = Global;
    $scope.lead = {name: ''};
    ...
    $scope.send = function (lead) {
        ...
        Leads.save(lead, function (response) {
            Global.modal.path = "/system/views/dialogs/success.html";
        });
    };
}]);